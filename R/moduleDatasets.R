...
datasetsServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # ---- master table (each row must have exactly 7 columns) ----
    df <- tibble::tribble(
      ...
      "Maddock ML (*)",   "",            "",           "iPSC-derived neural crest cells; sensory neurons; lower motor neurons",                                 "NCC: FRDA (N = 2, n = 4&ndash;5); IC (N = 2, n = 4); SN: FRDA (N = 2, n = 4&ndash;5); IC (N = 2, n = 4&ndash;5); LMN: FRDA (N = 1, n = 4); IC (N = 1, n = 4) &#167;",         "Shared by authors; accession pending",                                                                          "",
    ...
    output$datasets_tbl <- DT::renderDT({
      DT::datatable(
        df_disp,
        escape = FALSE,
        rownames = FALSE,
